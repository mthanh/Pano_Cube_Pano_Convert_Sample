...
def xyzcube_bottom(face_w):
    # out = np.zeros((face_w, face_w * 6, 3), np.float32) # 6 -> 1 -> process with bottom only
    out = np.zeros((face_w, face_w * 1, 3), np.float32)
    rng = np.linspace(-0.5, 0.5, num=face_w, dtype=np.float32)
    grid = np.stack(np.meshgrid(rng, -rng), -1)

    out[:, 0*face_w:1*face_w, [0, 2]] = grid
    out[:, 0*face_w:1*face_w, 1] = -0.5
...
def equirect_facetype_bottom(h, w):
    '''
    0F 1R 2B 3L 4U 5D
    '''
...
def sample_cubefaces_bottom(cube_faces, tp, coor_y, coor_x, order):
